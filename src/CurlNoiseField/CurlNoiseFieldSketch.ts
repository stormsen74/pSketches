...
export class CurlNoiseFieldSketch extends SketchTemplate {
    ...
    initGUI(): void {
        ...
        this.gui.add(this.settings, 'noiseScale', 0, .005, .0001).onChange(() => { this.sampleRate = .1 }).onFinishChange(() => { this.sampleRate = .1 })
        this.gui.add(this.settings, 'animateNoise').listen()
        this.gui.add(this.settings, 'useCurl').listen()
        this.gui.add(this.settings, 'noiseZ', 0, 1, .01).listen().onChange(() => { this.sampleRate = .1 }).onFinishChange(() => { this.sampleRate = .1 })
    ...
    draw() {
        ...
        if (this.settings.drawNoise) {
            ...
            if (this.hasBuffer) this.p.image(this.graphics, offsetX, offsetY);
        ...
        const updateRender = () => {
            ...
            for (let k = 0; k <= this.screenSize.x; k += sampleSize) {
                for (let j = 0; j <= this.screenSize.y; j += sampleSize) {
                    ...
                    if (this.settings.sw) {
                        // noise
                        usedColor = this.p.color(noise * 255, noise * 255, noise * 255, 200)
                    } else {
                        const vResult = this.curlField ? this.curlField(pScaled) : this.getVel(pScaled)
                        ...
                        usedColor = this.p.color(mappedHue, .5, lum, 1)
                    }
            ...
            // this.graphics.rect(0, 0, size.x, size.y)
